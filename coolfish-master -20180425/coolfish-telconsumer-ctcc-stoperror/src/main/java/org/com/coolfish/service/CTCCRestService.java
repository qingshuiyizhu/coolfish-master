...
public class CTCCRestService {
    ...
    private String queryStautsRequest(String tel, String url) {
        ...
        try {
            ...
            logger.error("电信号码[{}]解析查询号码状态响应消息出错,json-->{}, xml--> {}", tel, str, result);
        }
    ...
    public String getCumulateValue(String url, String tel) {
        ...
        try {
            ...
            if ("0".equals(status)) {
                ...
                logger.error("电信号码[{}]查询月套餐使用量出错,json-->{},xml --> {}", tel, str, result);
            }
        } catch (Exception e) {
            logger.error("电信号码[{}]解析月套餐使用量响应消息出错,json-->{},xml--> {}", tel, str, result);
        }
        return null;
    }
    ...
    public boolean getDWResp(String url, String tel) {
        ...
        try {
            ...
            if ("0".equals(rspType)) {
                logger.info("电信号码[{}]停机操作成功, json-->{},xml --> {}", tel, str, result);
                return true;
            } else if ("1".equals(rspType)) {
                String stop= object.get("result").getAsString();
                if("1999".equals(stop)) {
                    logger.error("电信号码[{}]|已存在停机类型为120000的停机记录，无法进行停机操作,json-->{}, xml --> {}", tel, str, result);
                return true;
                }               
            } else {
                logger.error("电信号码[{}]停机操作出错,json-->{}, xml --> {}", tel, str, result);
            }
        } catch (Exception e) {
            logger.error("电信号码[{}]解析停机操作响应消息出错, json-->{},xml --> {}", tel, str, result);
