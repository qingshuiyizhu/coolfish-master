...
public class CHARestService {
    ...
    public void LiuLiangControl(ChaOperator operator, MqJson json) {
        /*
         * ChaOperator operator = new ChaOperator("njplnxxjs", "ZGL@123456","487lhV125"); String
         * access_number = "1064973599126"; queryTraffic(operator,access_number,null);
         */
        // 得到使用量
        double value = -1;
        for (int i = 0; i < 3; i++) {
        ...
        if (value == -1) {
            ...
            if (CumulateValue > json.getSumflow()) {
                // 停机操作
                logger.info("单独断网操作：" + json.getTel());
                // 单独断网 号码 账号参数
                int result = -1;
                for (int j = 0; j < 3; j++) {
