...
public class MainService {
    ...
    public void start() {
        ...
        for (int i = 0; i < list.size(); i++) {
            ...
            Integer slient = 0;
            //是否沉默期
            if(1==card.getCard_status()&&1==card.getFrequency()) {
                slient=1;
            }
              if (1 == card.getType()) {
                // 月套餐
                ...
                json = new MqJson(card.getTel(), card.getOperatorid(), card.getZid(), sumFlow,
                        card.getUseflow().doubleValue(), card.getPer().doubleValue(),slient);
                if (1 == card.getOperator_type()) {
                    rabbitTemplate.convertAndSend("cmcc-monthly", JSON.toJSONString(json)+ new Date());
                    logger.info("加入移动月套餐队列：" + json);
                } else if (2 == card.getOperator_type()) {
                    rabbitTemplate.convertAndSend("cha-monthly", JSON.toJSONString(json)+ new Date());
                    logger.info("加入电信月套餐队列：" + json);
                } else if (3 == card.getOperator_type()) {
                    rabbitTemplate.convertAndSend("unicom-monthly", JSON.toJSONString(json)+ new Date());
            ...
            json = new MqJson(card.getTel(), card.getOperatorid(), card.getZid(), sumFlow,
                        card.getUseflow().doubleValue(), card.getPer().doubleValue(),slient);
                rabbitTemplate.convertAndSend("cmcc-total", JSON.toJSONString(json)+ new Date());
