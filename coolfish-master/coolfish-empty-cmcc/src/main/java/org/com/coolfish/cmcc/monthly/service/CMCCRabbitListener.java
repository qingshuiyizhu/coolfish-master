...
@Component
@RabbitListener(queues = "cmcc-empty")
public class CMCCRabbitListener {
    ...
    public void process(String message) {
        ...
        logger.info("移动空套餐队列(cmcc-empty)获取数据[{}]", message);
        MsisdnMessage msisdnMessage = JSON.parseObject(message, MsisdnMessage.class);
        long startTime = System.currentTimeMillis();
        logger.info("移动号码[{}]开始执行空套餐处理, 开始执行时间:{}, 时间戳:{}", msisdnMessage.getTel(), sdf.format(new Date(startTime)), startTime);
        handleMessage(msisdnMessage);
        long endTime = System.currentTimeMillis();
        logger.info("移动号码[{}]结束执行空套餐处理, 结束执行时间:{}, 时间戳:{}, 耗时:{}ms", msisdnMessage.getTel(), sdf.format(new Date(endTime)), endTime,
                (endTime - startTime));
    }
    ...
    public void handleMessage(MsisdnMessage msisdnMessage) {
        ...
        if (null == operator) {
            ...
            logger.info(msisdnMessage.getTel() + "调用移动空套餐处理服务,运营商账号信息：{}", operator.toString());
