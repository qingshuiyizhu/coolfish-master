...
//电信月套餐队列
@Component
@RabbitListener(queues = "ctcc-empty")
public class CTCCRabbitListener {
    ...
    public void process(String message) {
        ...
        logger.info("电信空套餐处理队列(cmcc-empty)获取数据[{}]", message);
        MsisdnMessage msisdnMessage = JSON.parseObject(message, MsisdnMessage.class);
       
        long startTime = System.currentTimeMillis();
        logger.info("电信号码[{}]开始执行空套餐处理, 开始执行时间:{}, 时间戳:{}", msisdnMessage.getTel(), sdf.format(new Date(startTime)), startTime);
        handleMessage(msisdnMessage);
        long endTime = System.currentTimeMillis();
        logger.info("电信号码[{}]结束执行空套餐处理, 结束执行时间:{}, 时间戳:{}, 耗时:{}ms", msisdnMessage.getTel(), sdf.format(new Date(endTime)), endTime,
                (endTime - startTime));
    ...
    public void handleMessage(MsisdnMessage msisdnMessage) {
        ...
        if (null == operator) {
            ...
            logger.info("[{}]调用电信执行空套餐处理服务,运营商账号信息：{}", msisdnMessage.getTel(), operator.toString());
            // 调用流量监控服务
