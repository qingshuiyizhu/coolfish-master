...
public class CMCCRabbitListener {
    ...
    public void process(String message) {
        ...
        logger.info("移动号码[{}]开始执行流量监控, 开始执行时间:{}, 时间戳:{}", msisdnMessage.getTel(), sdf.format(new Date(startTime)), startTime);
        handleMessage(msisdnMessage);
        long endTime = System.currentTimeMillis();
        logger.info("移动号码[{}]结束执行流量监控, 结束执行时间:{}, 时间戳:{}, 耗时:{}ms", msisdnMessage.getTel(), sdf.format(new Date(endTime)), endTime,
                (endTime - startTime));
    }
