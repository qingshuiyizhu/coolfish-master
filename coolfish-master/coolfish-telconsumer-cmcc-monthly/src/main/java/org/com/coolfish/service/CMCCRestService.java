...
public class CMCCRestService {
    ...
    public void handle(CMCCOperator operator, MsisdnMessage message) {
        ...
        // 2.1使用量为null 查询失败
        if (nowApiflow == null) {
            logger.warn("移动号码[{}]本月使用量获取失败，传回队列cmcc-monthly85", message.getTel());
    ...
    private String queryStauts(CMCCOperator operator, MsisdnMessage msisdnMessage) {
        ...
        try {
            ...
            logger.error("移动号码[{}]解析停机响应消息出错, xml --> {}", msisdnMessage.getTel(), result);
        }
    ...
    public String getCumulateValue(String tel, String xml) {
        ...
        try {
            ...
            if ("0000".equals(resp_code)) {
                ...
                logger.error("移动号码[{}]使用量查询操作出错, xml --> {}", tel, result);
            }
        } catch (Exception e) {

            logger.error("移动号码[{}]解析使用量查询响应消息出错, xml --> {}", tel, result);
        }
    ...
    public void disabledNumberController(CMCCOperator operator, MsisdnMessage message) {
        ...
        if (cutSuccess == true) {
            ...
            logger.info("移动号码[{}]停机成功", message.getTel());
        } else if (message.getRetry() < param.getRemax()) {
            // 停机失败，传到cmcc-monthly85队列
            message.setRetry(1 + message.getRetry());
            logger.info("移动号码[{}]停机失败，传到cmcc-monthly85队列", message.getTel());
            rabbitTemplate.convertAndSend("cmcc-monthly85", JSON.toJSONString(message).toString());
        } else {
            // 重试次数达到最大，传到email队列
            logger.info("移动号码[{}]停机失败重试次数达到最大，传到email队列", message.getTel());
    ...
    public boolean getDWResp(String tel, String xml) {
        ...
        try {
            ...
            if ("0".equals(ret_code)) {
                logger.info("移动号码[{}]停机成功, xml --> {}", tel, result);
                return true;
            } else {
