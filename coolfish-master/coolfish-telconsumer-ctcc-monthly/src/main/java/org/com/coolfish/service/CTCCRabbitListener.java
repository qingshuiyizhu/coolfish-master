...
public class CTCCRabbitListener {
    ...
    public void process(String message) {
        ...
        logger.info("电信月套餐队列(cmcc-monthly)获取数据[{}]", message);
        MsisdnMessage msisdnMessage = JSON.parseObject(message, MsisdnMessage.class);
       
        long startTime = System.currentTimeMillis();
        logger.info("电信号码[{}]开始执行流量监控, 开始执行时间:{}, 时间戳:{}", msisdnMessage.getTel(), sdf.format(new Date(startTime)), startTime);
        handleMessage(msisdnMessage);
        long endTime = System.currentTimeMillis();
        logger.info("电信号码[{}]结束执行流量监控, 结束执行时间:{}, 时间戳:{}, 耗时:{}ms", msisdnMessage.getTel(), sdf.format(new Date(endTime)), endTime,
                (endTime - startTime));
