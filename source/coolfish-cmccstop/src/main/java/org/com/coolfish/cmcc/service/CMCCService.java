...
public class CMCCService {
	...
	private Param param; 
	@Autowired
	private XMLHttpPost xMLHttpPost;
	// xml头部
	 ...
	 */
	public String UserstatuscontrolQuery(String msisdn, String SUB_SERVICE_STATUS, String service, String oprtype) {
		String desc = "物联网用户成员号码归属查询:" + "物联网号码msisdn: " + msisdn + ",SUB_SERVICE_STATUS:" + SUB_SERVICE_STATUS
				+ ",服务类型service:" + service + ",操作类型 oprtype:" + oprtype;
		logger.info(desc);
		// 报文
		...
		operationIn.setContent(new UserstatuscontrolContent(param.getGroupid(), msisdn, SUB_SERVICE_STATUS, service, oprtype));
		return CommonXML(operationIn, desc, UserstatuscontrolOperationIn.class);
	}
	...
	public String QryuserQuery(String telnum, String req_seq) {
		...
		return CommonXML(operationIn, desc, QryuserOperationIn.class);
	}
	 ...
	 * 请求参数 telnum:号码 req_seq:流水号
	 */
	public String QrystatusQuery(String telnum, String req_seq) {
		...
		return CommonXML(operationIn, desc, QrystatusOperationIn.class);
	}
	...
	public String QrycycleQuery(String telnum, String req_seq) throws Exception {
		...
		return CommonXML(operationIn, desc, QrycycleOperationIn.class);
	}
	...
	public String SuspendtestQuery(String telnum) throws Exception {
		...
		return CommonXML(operationIn, desc, SuspendtestOperationIn.class);
	}
	 ...
	 */
	public String QryuserinfoQuery(String ddr_city, String msisdn, String req_seq) throws Exception {
		String desc = "查询物联网用户基础信息及状态:" + "ddr_city:地市编码 " + ddr_city + ",物联网号码:" + msisdn + ",req_seq:流水号" + req_seq;
		logger.info(desc);
		// 报文
		...
		return CommonXML(operationIn, desc, QryuserinfoOperationIn.class);
	}
	 ...
	 */
	public String SLLZXJGQuery(String ddr_city, String orderid, String req_seq) throws Exception {
		String desc = "受理类执行结果查询:" + "ddr_city:地市编码 " + ddr_city + ",操作类型:" + orderid + ",req_seq:流水号" + req_seq;
		logger.info(desc);
		// 报文
		...
		return CommonXML(operationIn, desc, SLLZXJGOperationIn.class);
	}
	 ...
	 */
	public String tfjActionXml(String telnum, String oprtype, String reason) throws Exception {
		...
		return CommonXML(operationIn, desc, TFJOperationIn.class);
	}
	...
	public Future<String> queryDankaXml(String service_number, String cycle, String req_seq){
	    String desc = "物联网单卡实时流量查询:" + "查询号码为：" + service_number + ",查询月数为:" + cycle + ",req_seq:请求流水号:" + req_seq;
		logger.info(desc);
		// 报文
		...
		return new AsyncResult<String>(xml.toString()); 
	}
	...
	public String DTLLCQuery(String qrytype, String eccode, String cycle, String req_seq) {
		String desc = "物联网动态流量池实时情况查询:" + "qrytype：查询类型：" + qrytype + ",eccode ：客户编号:" + eccode + ",cycle:查询月份:" + cycle
				+ ",req_seq:请求流水号:" + req_seq;
		logger.info(desc);
		// 报文
		...
		// 请求类型
		operationIn.setReq_type("1");
		return CommonXML(operationIn, desc, DTLLCOperationIn.class);

	}

	public String CommonSend(Object operationIn, String desc, Class<?> clazz) {
		// 创建xStream对象
		XStream xStream = new XStream();
		// 设置别名, 默认会输出全路径
		...
		String json = "";
		try {
			String result = xMLHttpPost.sendXMLHttpPost(param.getUrl(), xml.toString());
			//String result = xMLHttpPost.sendXMLHttpPost1(param.getUrl(), xml.toString());
		 
			logger.info(desc + "返回的xml数据为:" + result);
			// 将xml转为json
			json = XmlTool.documentToJSONObject(result).toString();
			logger.info(desc + "返回的json数据为:" + json);
		} catch (Exception e) {
			e.printStackTrace();
		}
		return json;
	...
	//xml 拼接
	public String CommonXML(Object operationIn, String desc, Class<?> clazz) {
		// 创建xStream对象
		XStream xStream = new XStream();
		// 设置别名, 默认会输出全路径
		xStream.alias("operation_in", clazz);
